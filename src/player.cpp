  ...
  void create_playlist(const std::vector<std::string>& urls) {
    try {
        ...
        if (!playlist.empty()) {
            ...
            if (result < 0) {
                std::cerr << "Failed to load first track. Error code: " << result << std::endl;
                return;
            }
        }
    }
...
void next_trackk() {
    ...
    const char *cmd[] = {"loadfile", playlist[current_playlist_index].c_str(), NULL};
    mpv_command(mpv, cmd);
}

    // void playlist_next_track(std::vector<Track>& track_data, int& current_index) {
    //     std::lock_guard<std::mutex> lock(player_mutex);
    ...
    // void playlist_previous_track(std::vector<Track>& track_data, int& current_index) {
    //     std::lock_guard<std::mutex> lock(player_mutex);
        
    //     if (playlist.empty()) return;

    //     // Move to previous track in playlist
    //     current_playlist_index = (current_playlist_index - 1 + playlist.size()) % playlist.size();
