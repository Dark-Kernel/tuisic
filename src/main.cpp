...
auto fetch_main(const std::string &query) {
  ...
  // soundcloud.fetch_soundcloud_tracks(query).begin(),
  // soundcloud.fetch_soundcloud_tracks(query).end());
...
int main() {
  ...
  //         }
  // });
  // Component progress_slider = Slider( "Val", 0, track_data.size() - 1,
  // &selected);
  ...
  auto play_button = Button(&button_text, [&] {
    if (player->is_playing_state()) {
      ...
      // title = track_data[selected].artist + " - " +
  ...
  player->set_time_callback([&](double pos, double dur) {
    ...
    if (total_duration > 0) {
      ...
      // progress_percentage = std::min(100, std::max(0, static_cast<int>((pos /
      // dur) * 100.0)));
      screen.PostEvent(Event::Custom);
    }
  });
  ...
  //     if (total_duration > 0) {
  //         double seek_position = (progress_percentage / 100.0) *
  //         total_duration; seek_position = std::min(seek_position,
  //         total_duration);
  //         // seek_position = std::min(total_duration, std::max(0.0,
  //         seek_position)); player->seek(seek_position);
  //     }
  //     return true;
  // }
  // return false;
  ...
  progress_slider |= CatchEvent([&](Event event) {
    if (event == Event::Custom) {
      if (player->get_duration() > 0) {
        // double seek_pos = static_cast<double>(progress) / 100.0 *
        // player->get_duration();
